...
# http://0.0.0.0:8080/api_comentarios?user_hash=12345&action=put&id_comentario=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=0
    def put(self, id_post,fecha_comentario,comentario):
        try:
            config.model.insert_comentarios(id_post,fecha_comentario,comentario)
...
# http://0.0.0.0:8080/api_comentarios?user_hash=12345&action=update&id_comentario=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_comentario, id_post,fecha_comentario,comentario):
        try:
            config.model.edit_comentarios(id_comentario,id_post,fecha_comentario,comentario)
            comentarios_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(comentarios_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.put(id_post,fecha_comentario,comentario)
                elif action == 'delete':
                    return self.delete(id_comentario)
                elif action == 'update':
                    return self.update(id_comentario, id_post,fecha_comentario,comentario)
            else:
                raise web.seeother('/404')
        except Exception as e:
