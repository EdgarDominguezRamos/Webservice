...
# http://0.0.0.0:8080/api_guardado?user_hash=12345&action=put&id_guardado=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=0
    def put(self, id_usuario_eco,id_post,categoria):
        try:
            config.model.insert_guardado(id_usuario_eco,id_post,categoria)
            guardado_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(guardado_json)
        except Exception as e:
...
# http://0.0.0.0:8080/api_guardado?user_hash=12345&action=update&id_guardado=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_guardado, id_usuario_eco,id_post,categoria):
        try:
            config.model.edit_guardado(id_guardado,id_usuario_eco,id_post,categoria)
            guardado_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(guardado_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.put(id_usuario_eco,id_post,categoria)
                elif action == 'delete':
                    return self.delete(id_guardado)
                elif action == 'update':
                    return self.update(id_guardado, id_usuario_eco,id_post,categoria)
            else:
                raise web.seeother('/404')
        except Exception as e:
